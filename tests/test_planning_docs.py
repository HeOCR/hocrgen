...
def test_planning_docs_agree_on_current_and_next_notation() -> None:
    ...
    assert f"| D2 | Expansion and benchmark formation | D2a | Stable source-operations maturity | completed |" in roadmap
    assert f"| D3 | Expansion and benchmark formation | D3a | Benchmark subset v1 | next |" in roadmap
    ...
    assert "The next planned milestone is `D3a`" in Path("docs/pre_alpha_freeze_plan.md").read_text(encoding="utf-8")
