...
CURRENT_COMPLETED_NOTATION = "F1b2"
PLANNING_FILES = [
    Path(".agent-plan.md"),
    Path("README.md"),
...
def test_planning_docs_agree_on_current_and_next_notation() -> None:
    ...
    assert "next implementation should wait for `F1c` until the remaining `F1b2` source-depth count gaps are resolved" in agent_plan
    ...
    assert "| F1 | Beta-scale acquisition trial | F1a, F1b, F1b2, F1c, F1d |" in roadmap
