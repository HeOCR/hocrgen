...
- exercising target-scale inventory through the gates for the `27` NLI / `27` Pinkas / `26` BiblIA real-source target now that NLI has `27 / 27` real runnable/cached fixtures
- bounded operator-only beta acquisition execution for `80` real items plus `80` synthetic controls after target-scale trial artifacts preserve the required gates
