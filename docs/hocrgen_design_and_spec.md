...
F5a defines the readiness contract that must pass before this publishing stage is invoked for a mixed `HeOCR` public beta. F5b implements local blocked packaging and handoff verification, not final publication. F5c adds blocker-closure sequencing and repo-owned handoff gap reporting without changing publication side effects. F5d adds item/status-level repo-owned blocker evidence for privacy/review, benchmark references, and takedown/private reporting while preserving external source-depth and synthetic-scale blockers. F6a adds the planning-only closure roadmap for those blockers. F6b records verified GitHub private vulnerability reporting evidence for `HeOCR/hocrgen`, allowing only the takedown/private reporting gate to pass while the blocked export remains blocked on unrelated gates. F6c evaluates benchmark-reference readiness and keeps `benchmark_references` blocked because current reference coverage is partial (`1 / 3` selected benchmark items reviewed/adjudicated). F6d evaluates privacy/review readiness and keeps `privacy_review` blocked because review-required item evidence remains default-unresolved and no repo-tracked review/config/source-status closure evidence exists. F6e evaluates source-depth/composition readiness and keeps `source_depth_composition` blocked because current public-profile payload evidence is only `2 / 80` real candidates; F1c/operator-only/source-depth-only inventory remains disclosure evidence, not public payload readiness. F6f1 records the hocrsyngen installed-CLI adapter preflight plan without runtime behavior changes and preserves the finding that raw public hocrsyngen manifests are not current hocrgen release-path inputs until hocrgen owns the missing release/import metadata form. F6f2a implements the operator-only hocrgen reader for hocrsyngen evidence-run roots and emits diagnostics with `release_eligible: false`, without feeding `project_synthetic` or any release/export command. F6f2 defines the hocrgen-owned import metadata packet for provider identity/version, rendering, and Hebrew coverage metadata; validates the packet with a typed hocrgen-owned schema; records release/import projection as false unless explicit provider-runtime evidence exists; and still keeps generated batches out of release paths. Operator trial success is not enough: public publishability also requires source-depth/composition evidence, validated synthetic target scale and cap policy, rights/provenance closure, privacy/review closure, duplicate and leakage clearance, benchmark-reference status, annotation-status disclosure, release-relative manifest/checksum/archive portability, public dataset/provenance/changelog docs, and takedown/removal readiness.
...
- provider metadata for fixture-backed hocrsyngen batches, including offline manifest-batch mode and explicit no-network/no-REST/no-GPU/no-LLM/no-diffusion flags; operator preflight sidecars must keep those flags unproven until a provider-runtime evidence contract supplies them
...
F5d materializes repo-owned closure evidence as `manifests/public_beta_repo_owned_blocker_report.json`. The report lists unresolved review/privacy item ids and reasons, benchmark-reference readiness by public status and adjudication state, takedown/private-reporting configuration, and repository settings-check evidence. F6e adds source-depth/composition evidence to the blocker reporting surface and writes `manifests/public_beta_source_depth_composition_report.json`, which records real public-profile payload counts separately from F1c/operator-only/source-depth-only inventory while keeping source-depth/composition input-dependent until real candidates are promoted. It is an action report, not a readiness override: draft or unavailable benchmark references stay blocked, unresolved review/privacy items stay blocked, and operator-only source-depth evidence stays blocked until public-profile candidates pass normal gates.

F6a defines the post-F5 closure roadmap without changing runtime behavior. F6b records the verified private reporting configuration for takedown readiness in `src/hocrgen/config/public_beta.yaml`, using safe repository settings-check metadata from the authenticated GitHub API. That evidence can pass only the takedown gate and must not affect source-depth, synthetic-scale, benchmark-reference, review/privacy, or portability gates. F6c evaluates benchmark-reference readiness without changing the contract: the current selected benchmark set has one reviewed/adjudicated reference, one draft/in-review reference, and one unavailable reference, so the blocker remains blocked and the limitation is disclosed in the readiness and repo-owned blocker reports. F6d evaluates privacy/review readiness without changing the contract: governed candidates remain review-required with default-unresolved review decisions, and no repo-tracked review decision, privacy config change, or source-status change closes the gate, so the blocker remains blocked and the limitation plus source-status evidence are disclosed in the repo-owned blocker report. F6e evaluates source-depth/composition readiness without changing the contract: current public-profile payload evidence has only one NLI item, one Pinkas item, no BiblIA items, and two synthetic items, so the blocker remains blocked and the limitation is disclosed in the readiness, source-depth composition, and repo-owned blocker reports. F6f1 is planning-only and defines an explicit operator-only preflight that may call installed hocrsyngen CLI JSON surfaces to validate `generation_report.v1`, `validation_report.v1`, `template_catalog.v2`, public `generation_manifest.v1`, assets, SHA-256 hashes, and catalog joins. F6f2a implements the read-only evidence-root side of that preflight in hocrgen: it validates `candidate_evidence_run_report.v1`, retained reports, SHA-256 inventory, public manifest fields, assets, and catalog joins, and writes a diagnostic report with `release_eligible: false`. F6f2 defines `hocrgen_hocrsyngen_import_metadata_packet.v1`, a hocrgen-owned sidecar/import packet that derives provider identity/version, rendering metadata, and Hebrew coverage metadata from the validated evidence root, validates the packet itself with typed hocrgen models, and reports release/import model projection separately. Current evidence roots without `candidate_evidence_run_report.provider_runtime` do not prove no-network/no-REST/no-GPU/no-LLM/no-diffusion flags, so their projection remains false even when the sidecar schema is valid. That packet must not wire raw hocrsyngen output into release paths or require hocrsyngen to mutate manifest v1. Later F6 readiness work must close remaining gates only with real evidence: synthetic-scale release-path admission, privacy/review closure, source-depth evidence, and final convergence evidence. Partial or unavailable benchmark coverage, unresolved review evidence, and operator-only source-depth evidence may be disclosed as limitations, but they must not become passing readiness gates unless a separate governance PR explicitly changes the public beta contract. F6 does not permit hocrgen to import hocrsyngen internals, call hocrsyngen commands from default release/export paths, use network services, or add GPU/LLM/diffusion dependencies.
...
- synthetic target scale and caps: a validated hocrgen-owned release/import metadata packet with true release/import projection plus a larger validated hocrsyngen `generation_manifest.v1` batch admitted through release paths for the planned synthetic-control target, plus continued enforcement of public-profile/export cap policy
...
- a repo-owned blocker report that gives privacy/review, benchmark-reference, and takedown/private-reporting evidence precise closure status without changing unrelated gate semantics
