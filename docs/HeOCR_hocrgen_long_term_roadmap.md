...
The immediate implementation critical path after `F6c` is:
...
- F6d can progress independently when its repo-owned evidence exists; F6b now has verified private-reporting evidence and F6c has current benchmark-reference limitation reporting, while F6g should wait for remaining repo-owned closure plus external/input-dependent source-depth and synthetic-scale inputs
