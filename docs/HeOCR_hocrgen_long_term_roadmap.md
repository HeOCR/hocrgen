...
The immediate acquisition implementation critical path after `F1c` is:
...
2. Add `F1d` near-duplicate, source-group, and split-leakage hardening before treating the trial as a path toward larger public beta or release-candidate export work.
3. Keep F2 benchmark ground-truth work, F3 modern handwritten acquisition, F4 synthetic-provider gates, and F5 publication readiness as separate follow-on milestones.
