...
| F1c | F1 | Execute bounded beta-trial acquisition artifacts for `80` real items and `80` synthetic controls | no | planned | after target-scale inventory is exercised through pipeline gates |
...
1. Execute `F1c` target-scale operator-trial artifacts over the `27` NLI / `27` Pinkas / `26` BiblIA real-source target plus `80` synthetic controls.
2. Treat `F1c` as blocked until source-depth, rights, privacy, review, dedupe, split, benchmark, synthetic-cap, source-health, and export-portability gates remain enforceable at the target scale.
...
- NLI runnable/cached source depth and Pinkas/BiblIA/static synthetic target-scale inventory gaps are closed before `F1c`; target-scale gate execution remains the next source-depth prerequisite
