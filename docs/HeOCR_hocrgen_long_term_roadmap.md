...
| F1c | F1 | Execute bounded beta-trial acquisition artifacts for `80` real items and `80` synthetic controls | no | planned | after F1b source-depth gaps are resolved |
...
The immediate acquisition implementation critical path after `F1b` remains:

1. Resolve the `F1b` source-depth gaps before executing the `F1c` target-scale trial.
...
`F1b` is implemented on the current ref as an operator-only discovery artifact. Every `discover` run writes `source_depth_feasibility.json` with the F1 target counts, per-source allocation, runnable/cached candidate counts, asset counts, exploratory/live candidate counts where applicable, gaps, feasibility status, operator notes, required gates, and non-goals. The current fixture-backed depth is `7` runnable NLI seeds plus `14` exploratory NLI catalog entries, `1` Pinkas record, and `1` BiblIA record. NLI needs operator promotion before the `27` item target is feasible from committed fixtures. Pinkas and BiblIA explicitly remain not feasible for target scale until source-depth expansion is defined, fixture-backed, rights-safe, and reviewable. This artifact does not create broad crawling, public beta export, release-candidate export, publication, or network-dependent CI.
