...
| F2 | Benchmark ground-truth foundation | F2a, F2b | Transcription/layout guidelines, reference manifests, benchmark references, and adjudication workflow | in progress |
...
| F4c | F4 | Add deeper Hebrew rendering/provider metadata gates for `hocrsyngen` batches | no | planned | after F4b |
...
The immediate acquisition implementation critical path after `F2a` is:
...
### Current-ref implementation
`F2a` is implemented on the current ref as human-facing benchmark ground-truth guidance, not as runtime ingestion or scoring-gate work. [`docs/benchmark_ground_truth_guidelines.md`](./benchmark_ground_truth_guidelines.md) defines Hebrew transcription policy for logical text order, Unicode NFC normalization, right-to-left and bidi behavior, niqqud, punctuation, Hebrew/Arabic/Latin numerals, Latin fragments, abbreviations, uncertain or damaged text, marginal and deleted text, and line/page boundaries. It also defines layout-label conventions for page, region, line, and optional word/reference levels; pixel coordinates over normalized release assets; reading order; multi-page items; uncertainty/review flags; and portability constraints. The same document records the documentation-level `benchmark_reference_manifest.v1` contract, including release-relative paths, item/source identity linkage, transcription and layout-label references, reviewer/adjudication status, correction/versioning fields, and separate public, private adjudication, and future hidden reference classes. `F2b` remains planned for benchmark-reference ingestion, adjudication artifacts, and benchmark versioning gates.
