...
- **HeOCR**: the public mixed real+synthetic dataset
...
- **hocrsyngen**: the synthetic Hebrew OCR/HTR generation package that emits candidate generated sample units and manifests consumed by `hocrgen`
- **hletterscript**: the upstream per-writer Hebrew letter-glyph image dataset that feeds `hocrsyngen`
- **hletterscriptgen**: the upstream framework that crops scans into per-letter glyph variants under the `letter_set.v1` contract
- **public-domain-hand-written-hebrew-scans**: the upstream rights-clean modern-handwriting page-scan corpus that anchors the synthetic provenance chain

`F4a` records the four-repository synthetic spinout boundary (`hocrsyngen`, `hocrgen`, `HeOCR`, `HeOCRsynth`). `F4f` extends that boundary upstream to recognize the additional HeOCR ecosystem repositories that feed `hocrsyngen` without changing any hocrgen runtime behavior, readiness gate, or release path. See [`heocr_ecosystem_overview.md`](./heocr_ecosystem_overview.md) for the full chain.
...
| F4 | External synthetic provider integration | F4a, F4b, F4c, F4d, F4e, F4f | Synthetic spinout architecture, provider manifest contract, fixture-backed adapter, Hebrew rendering/provider gates, HeOCRsynth export handoff, shared release export packaging primitives, and wider HeOCR ecosystem upstream-chain documentation | completed |
...
| F6 | Public beta closure and external input integration | F6a, F6b, F6c, F6d, F6e, F6f1, F6f2a, F6f2, F6g | Post-F5 closure roadmap, takedown/private reporting closure, benchmark-reference evidence review, remaining repo-owned blocker closure, source-depth evidence, hocrsyngen installed-CLI adapter preflight planning, operator-only evidence-root reader, hocrgen-owned metadata sidecar definition, and final readiness rerun | partial |
...
| F4f | F4 | Document the wider HeOCR ecosystem upstream chain without changing runtime behavior or release gates | no | completed | current ref documentation-only HeOCR ecosystem overview PR; extends F4a four-repository spinout boundary upstream to `public-domain-hand-written-hebrew-scans`, `hletterscriptgen`, and `hletterscript` |
...
| F6f2 | F6 | Define hocrsyngen release/import metadata sidecar | no | completed | current ref hocrgen_hocrsyngen_import_metadata_packet.v1 derived from validated evidence roots; operator-only release_eligible=false |
...
The immediate implementation critical path after `F4f` is:
...
8. Execute the remaining F6 work in evidence order: `F6f2` has settled the downstream release/import metadata form as an operator-only hocrgen-owned packet, and `F6g` may rerun public beta readiness only after privacy/review closure, source-depth, and synthetic-scale release-path evidence exists.
...
- `F4f`: document the wider HeOCR ecosystem upstream chain (`public-domain-hand-written-hebrew-scans`, `hletterscriptgen`, `hletterscript`) that feeds `hocrsyngen` without changing runtime behavior, readiness gates, or release paths

### Current-ref implementation
`F4a` is implemented on the current ref as planning and documentation only. `F4b` adds fixture-backed hocrsyngen manifest ingestion. `F4c` hardens the hocrgen-side fixture/import boundary with required provider metadata, dependency-boundary flags, RTL/logical rendering metadata, Hebrew coverage validation, source-health signals, config validation, and synthetic composition reporting without redefining public hocrsyngen `generation_manifest.v1`. `F4d` adds `hocrgen export-synthetic`, a synthetic-only HeOCRsynth handoff path that selects release-ready `PROJECT-SYNTHETIC` items from existing hocrgen pipeline state, preserves synthetic disclosure and hocrgen-side provider/rendering/Hebrew coverage metadata for hocrsyngen-backed items, writes payload assets under `data/synthetic/<split>/<item_id>/`, and keeps mixed `HeOCR` `export-alpha` releases distinct. `F4e` extracts shared release export packaging primitives into `hocrgen.package.common` so alpha and HeOCRsynth exporters share release-relative portable payload shaping, stats, review/audit payloads, benchmark/reference filtering, release diffs, changelog rendering, and standard manifest/doc writing while retaining separate mixed `HeOCR` and synthetic-only release policy. F4d/F4e do not add acquisition code, hocrsyngen runtime calls, new hocrgen generator dependencies, or changes to current mixed public/alpha selection behavior. `F4f` is implemented on the current ref as documentation only. It records the wider HeOCR ecosystem upstream chain that surrounds the original `F4a` four-repository spinout boundary: `public-domain-hand-written-hebrew-scans` is the upstream rights-clean modern-handwriting page-scan corpus (per-scan rights evidence in `data/index/entries.jsonl`), `hletterscriptgen` is the framework that crops those scans into per-letter glyph variants under the `letter_set.v1` contract, and `hletterscript` is the per-writer glyph-image dataset that ultimately feeds `hocrsyngen` synthetic page composition. `F4f` is recorded in [`heocr_ecosystem_overview.md`](./heocr_ecosystem_overview.md), the README, the synthetic asset contribution guide, the `.agent-plan.md` summary, and the design/spec discussion of the synthetic provider boundary. It does not change `project_synthetic`, `build-release`, `export-alpha`, `export-synthetic`, `export-public-beta`, `hocrgen hocrsyngen-preflight`, the `hocrgen_hocrsyngen_import_metadata_packet.v1` contract, any source-adapter contract, the public beta readiness report, the blocker-closure plan, the repo-owned blocker report, the source-depth/composition report, takedown/private reporting evidence, or any synthetic cap, profile, or release gate. hocrgen does not import code from `public-domain-hand-written-hebrew-scans`, `hletterscriptgen`, `hletterscript`, or `hocrsyngen`, does not call upstream CLIs from default release/export commands, and does not re-validate upstream rights, glyph extraction, or per-image checksums; those responsibilities stay upstream and are inherited as per-item rights/provenance disclosure through the existing hocrgen item manifests. `F4f` does not affect the hard `2 / 80` synthetic target-scale blocker, the source-depth/composition blocker, the benchmark-reference blocker, or the privacy/review blocker.
...
- hocrgen-owned release/import metadata sidecar definition before any larger validated hocrsyngen batch is admitted to release paths, not through hocrsyngen private internals
...
7. `F6f2` settles the hocrgen-owned release/import metadata form for provider, rendering, and Hebrew coverage metadata as an operator-only sidecar/import packet. It cannot close synthetic target scale by itself, and the current `2 / 80` release-path evidence remains blocked until a larger validated batch is admitted through hocrgen gates.
8. `F6g` can run final public beta readiness convergence only after privacy/review closure, source-depth, and synthetic-scale release-path evidence exists. It must still leave the export blocked if any F5a/F5b gate fails.
...
- `F6f2` defines and validates the hocrgen-owned import metadata sidecar without mutating hocrsyngen manifests or admitting the batch into release/export paths
...
- a larger hocrsyngen batch must be admitted through hocrgen release-path gates before synthetic target scale can pass
...
- F6 depends on F5 blocker reporting remaining truthful; `F6e` depends on real public-profile source-depth/composition evidence, `F6f1` depends on hocrsyngen S6 public CLI handoff evidence and hocrgen-side adapter planning, `F6f2a` depends on hocrsyngen evidence-run roots remaining public-boundary evidence, and `F6f2` depends on hocrgen deriving and validating a hocrgen-owned release/import metadata sidecar without mutating hocrsyngen manifest v1 or changing release eligibility
...
- F6b now has verified private-reporting evidence, F6c has current benchmark-reference limitation reporting, F6d has current privacy/review limitation reporting, F6e has current source-depth/composition limitation reporting, F6f1 has hocrsyngen installed-CLI adapter preflight planning, F6f2a has an operator-only hocrsyngen evidence-root reader, and F6f2 has a hocrgen-owned import metadata sidecar; F6g should wait for privacy/review closure plus real source-depth and synthetic-scale release-path inputs
