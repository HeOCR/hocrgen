...
The immediate acquisition implementation critical path after `F2b` is:
...
`F2b` is implemented on the current ref as optional runtime ingestion and validation for those contracts. The packaged fixture under `src/hocrgen/data/benchmark/benchmark_v1/reference_manifest.json` exercises reviewed, draft, and unavailable reference states. `build-release` validates reference-manifest, transcription-reference, and layout-reference shapes; rejects non-portable absolute, `file://`, `.work`, backslash, and path-traversal paths; checks benchmark item/source/split linkage; verifies layout asset checksum/dimension linkage against current normalized assets; emits `benchmark_reference_manifest.json`, `benchmark_reference_status.json`, and `benchmark_reference_versioning.json`; and mirrors those artifacts through `export-alpha` for exported benchmark items. This does not make transcriptions or layout labels mandatory for current public or alpha outputs, does not change `benchmark_v1` membership, and does not resolve the F1d benchmark/holdout leakage risk or public beta/export readiness.
