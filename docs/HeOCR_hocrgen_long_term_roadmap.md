...
The immediate implementation critical path after `F6e` is:
...
- F6b now has verified private-reporting evidence, F6c has current benchmark-reference limitation reporting, F6d has current privacy/review limitation reporting, and F6e has current source-depth/composition limitation reporting; F6g should wait for privacy/review closure plus real source-depth and synthetic-scale inputs
