...
| F1 | Beta-scale acquisition trial | F1a, F1b, F1b2, F1b3, F1c, F1d | Operator-only beta-scale plan, source-depth feasibility/reporting, static-source expansion feasibility, source-depth count expansion, bounded trial artifacts, and near-duplicate/leakage hardening | in progress |
...
| F1c | F1 | Execute bounded beta-trial acquisition artifacts for `80` real items and `80` synthetic controls | no | planned | after F1b3 NLI promotion gap is resolved and target-scale inventory can be exercised through pipeline gates |
...
1. Resolve the remaining `F1b3` NLI promotion gap and exercise target-scale inventory through pipeline gates before treating `F1c` as unblocked.
...
- Pinkas/BiblIA/static synthetic target-scale inventory gaps are closed before `F1c`; NLI promotion and target-scale gate execution remain the remaining source-depth prerequisites
...
- NLI may not have enough promoted runnable/cached source depth for the even-mix target
