...
| F1 | Beta-scale acquisition trial | F1a, F1b, F1b2, F1c, F1d | Operator-only beta-scale plan, source-depth feasibility/reporting, static-source expansion feasibility, bounded trial artifacts, and near-duplicate/leakage hardening | in progress |
...
| F1c | F1 | Execute bounded beta-trial acquisition artifacts for `80` real items and `80` synthetic controls | no | planned | after F1b2 source-depth count gaps are resolved |
...
The immediate acquisition implementation critical path after `F1b2` remains:

1. Resolve the remaining `F1b2` source-depth count gaps before executing the `F1c` target-scale trial.
