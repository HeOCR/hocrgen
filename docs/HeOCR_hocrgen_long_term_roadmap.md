...
The immediate implementation critical path after `F3b` is:
...
5. Move next to `F4c` hocrsyngen rendering/provider metadata gates; keep F4d synthetic-only export handoff and F5 publication readiness as separate follow-on milestones.
