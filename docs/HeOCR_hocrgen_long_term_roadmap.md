...
| F3 | Modern handwritten acquisition program | F3a, F3b | Rights-clean modern Hebrew handwriting collection policy and operator acquisition workflow | partial |
...
The immediate acquisition implementation critical path after `F3a` is:
...
5. Move next to `F3b` bounded modern handwritten acquisition workflow and typed intake manifests; keep F4c/F4d synthetic follow-ups and F5 publication readiness as separate follow-on milestones.
...
### Current F3a policy boundary
`F3a` is a policy foundation, not an acquisition implementation. It establishes contributor consent, public-use release terms, rights provenance, contemporary privacy screening, takedown/removal handling, scanning/upload standards, mandatory operator review, composition targets, and the boundary between modern real handwriting, historical public sources, and synthetic data. `F3b` is still required before collecting modern handwriting samples into this repository.

### Exit criteria
- modern handwritten samples can be collected through the future F3b workflow without weakening privacy or rights posture
