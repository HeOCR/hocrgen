...
| F1 | Beta-scale acquisition trial | F1a, F1b, F1b2, F1b3, F1b4, F1c, F1d | Operator-only beta-scale plan, source-depth feasibility/reporting, static-source expansion feasibility, source-depth count expansion, NLI runnable/cached promotion, bounded trial artifacts, and near-duplicate/leakage hardening | in progress |
...
| F1b4 | F1 | Expand NLI runnable/cached source depth before F1c execution | no | planned | after real NLI source-cached fixtures are captured; generated stand-ins must not count |
| F1c | F1 | Execute bounded beta-trial acquisition artifacts for `80` real items and `80` synthetic controls | no | planned | after F1b4 NLI promotion gap is resolved and target-scale inventory can be exercised through pipeline gates |
...
`F1b` is implemented on the current ref as an operator-only discovery artifact. Every `discover` run writes `source_depth_feasibility.json` with the F1 target counts, per-source allocation, observed candidate counts, health-eligible runnable/cached candidate counts, asset counts, exploratory catalog counts where applicable, gaps, feasibility status, report-scoped warnings, operator notes, required gates, and non-goals. The current fixture-backed NLI depth is `7` runnable/cached NLI seeds plus `20` exploratory NLI catalog entries. NLI needs operator promotion into real source-cached fixtures before the `27` item target is feasible from committed fixtures. This artifact does not create broad crawling, public beta export, release-candidate export, publication, or network-dependent CI.
...
`F1b3` is implemented on the current ref as source-depth inventory expansion before F1c execution. Pinkas now has `27 / 27` packaged source-depth inventory records/assets from bounded public-domain Commons page fixtures, BiblIA has `26 / 26` packaged source-depth inventory records/assets from bounded public-domain `Bible from 1300` page fixtures, and `project_synthetic` is configured with `80 / 80` target-scale operator-reporting candidates. The report keeps this target-scale inventory separate from runnable/cached candidates: normal discovery still sees `1` Pinkas record, `1` BiblIA record, and `2` synthetic controls until an operator trial explicitly exercises target-scale artifacts through the pipeline gates.

`F1b4` is planned on the current ref as the NLI runnable/cached source-depth promotion before F1c execution. NLI remains at `7 / 27` fixture-backed runnable/cached seeds; the exploratory catalog now has `20` manually vetted candidate URLs, including six added Any Use Permitted candidates, but those entries do not count toward runnable/cached source depth until the operator promotion path captures real NLI metadata/assets into committed local fixtures. Generated or hand-authored stand-in assets must not close the F1 source-depth gate. `F1c` remains blocked until NLI source depth is real-cached and target-scale operator-trial artifacts exercise NLI, Pinkas, BiblIA, and synthetic inventory through rights, privacy, review, dedupe, split, benchmark, synthetic-cap, source-health, and export-portability gates.
...
- Pinkas/BiblIA/static synthetic target-scale inventory gaps are closed before `F1c`; NLI runnable/cached promotion and target-scale gate execution remain source-depth prerequisites
...
- target-scale trial execution may still expose acquisition, review, or portability failures despite source-depth feasibility
