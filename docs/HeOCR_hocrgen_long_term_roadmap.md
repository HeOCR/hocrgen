...
| F1b4 | F1 | Expand NLI runnable/cached source depth before F1c execution | no | completed | current ref NLI promotion PR |
| F1c | F1 | Execute bounded beta-trial acquisition artifacts for `80` real items and `80` synthetic controls | no | planned | after target-scale inventory can be exercised through pipeline gates |
...
The immediate acquisition implementation critical path after `F1b4` remains:

1. Exercise target-scale inventory through pipeline gates before treating `F1c` as unblocked.
...
`F1b4` is implemented on the current ref as NLI runnable/cached source-depth promotion before F1c execution. NLI now has `27 / 27` fixture-backed runnable/cached seeds by promoting all current exploratory catalog entries plus six manually selected Any Use Permitted NLI candidates into committed local fixtures/assets. The exploratory catalog is empty after this promotion. `F1c` remains blocked until target-scale operator-trial artifacts exercise NLI, Pinkas, BiblIA, and synthetic inventory through rights, privacy, review, dedupe, split, benchmark, synthetic-cap, source-health, and export-portability gates.
...
- Pinkas/BiblIA/static synthetic target-scale inventory gaps and NLI runnable/cached promotion are closed before `F1c`; target-scale gate execution remains the remaining source-depth prerequisite
