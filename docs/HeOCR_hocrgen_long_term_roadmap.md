...
| F6 | Public beta closure and external input integration | F6a, F6b, F6c, F6d, F6e, F6f1, F6f2, F6g | Post-F5 closure roadmap, takedown/private reporting closure, benchmark-reference evidence review, remaining repo-owned blocker closure, source-depth evidence, hocrsyngen installed-CLI adapter preflight planning, later larger hocrsyngen batch integration, and final readiness rerun | partial |
...
The immediate implementation critical path after `F6f1` is:
...
5. `F6f1` is planning-only and cannot close synthetic target scale. It can only define a future operator-only preflight that calls installed hocrsyngen CLI JSON surfaces, validates public reports/manifests/assets/catalog joins, recomputes hashes, reports missing hocrgen-owned release/import metadata, and leaves raw hocrsyngen output out of the default release path.
6. `F6f2` can close synthetic target scale only after a larger validated hocrsyngen `generation_manifest.v1` batch exists and hocrgen has a settled downstream release/import metadata form for provider, rendering, and Hebrew coverage metadata. The current `2 / 80` evidence remains blocked until then.
7. `F6g` can run final public beta readiness convergence only after privacy/review closure, source-depth, and synthetic-scale inputs exist. It must still leave the export blocked if any F5a/F5b gate fails.
...
- F6 depends on F5 blocker reporting remaining truthful; `F6e` depends on real public-profile source-depth/composition evidence, `F6f1` depends on hocrsyngen S6 public CLI handoff evidence and hocrgen-side adapter planning, and `F6f2` additionally depends on a larger validated hocrsyngen batch existing outside hocrgen plus a settled hocrgen-owned release/import metadata form before integration
...
- F6b now has verified private-reporting evidence, F6c has current benchmark-reference limitation reporting, F6d has current privacy/review limitation reporting, F6e has current source-depth/composition limitation reporting, and F6f1 has hocrsyngen installed-CLI adapter preflight planning; F6g should wait for privacy/review closure plus real source-depth and synthetic-scale inputs
