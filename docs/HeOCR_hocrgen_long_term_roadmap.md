...
| F1c | F1 | Execute bounded beta-trial acquisition artifacts for `80` real items and `80` synthetic controls | no | planned | after F1b3 NLI source-depth count gap is resolved |
...
1. Resolve the remaining `F1b3` NLI source-depth count gap before executing the `F1c` target-scale trial.
...
`F1b3` is implemented on the current ref as source-depth count expansion before F1c execution. Pinkas now has `27 / 27` packaged records/assets from bounded public-domain Commons page fixtures, BiblIA has `26 / 26` packaged records/assets from bounded public-domain `Bible from 1300` page fixtures, and `project_synthetic` is configured at `80 / 80` operator-reporting candidates. The added Pinkas/BiblIA expansion records are marked as F1 source-depth-only so they count for operator feasibility without automatically changing normal release/export discovery. `F1c` remains blocked because NLI still has only `7 / 27` runnable/cached fixture-backed seeds; the `14` exploratory catalog entries still require operator promotion before they count.
...
- Pinkas/BiblIA/static synthetic source-depth count gaps are closed before `F1c`; NLI promotion remains the remaining source-depth prerequisite
